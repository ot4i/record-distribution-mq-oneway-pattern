...
public class _jet_RecordDistributoresql implements JET2Template {
    private static final String _jetns_c = "org.eclipse.jet.controlTags"; //$NON-NLS-1$

    public _jet_RecordDistributoresql() {
        super();
    }
    ...
    public void generate(final JET2Context context, final JET2Writer __out) {
        ...
        out.write(NL);         
        out.write("CREATE DATABASE MODULE SaveUDPs");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tCREATE FUNCTION Main() RETURNS BOOLEAN");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tBEGIN");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("-- This module saves the logging UDPs to the environment to make them available to any subflows");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("-- UDPs are handled this way (rather than by promotion) to enable subflow substitution ");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("-- without imposing UDP definitions");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tSET Environment.PatternVariables.ErrorLoggingOn = ErrorLoggingOn;");  //$NON-NLS-1$        
        out.write(NL);         
